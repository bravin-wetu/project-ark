@section('title', 'New Requisition - ' . $project->name)
...
<x-workspace-layout :workspace="$project" :workspaceType="'projects'">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-2 text-sm text-smoke-500 mb-2">
            ...
            <span class="text-ink-900 font-medium">New Requisition</span>
        </div>
        <h1 class="text-2xl font-semibold text-ink-900">Create Requisition</h1>
        <p class="text-smoke-600 mt-1">Request procurement of goods or services against a budget line.</p>
    </div>

    <form action="{{ route('projects.requisitions.store', $project) }}" method="POST" id="requisitionForm">
        @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" 
                                      ...
                                      placeholder="Detailed description of what is being requested...">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="justification" 
                                      id="justification" 
                                      rows="2"
                                      class="input-field @error('justification') border-red-500 @enderror"
                                      placeholder="Why is this procurement necessary?">{{ old('justification') }}</textarea>
                            @error('justification')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex justify-end">
                            <div class="w-64">
                                <div class="flex justify-between py-2">
                                    ...
                                    <span class="text-lg font-semibold text-ink-900" id="estimatedTotal">$0.00</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                            ...
                            <select name="budget_line_id" 
                                ...
                                @foreach($budgetLines as $line)
                                    <option value="{{ $line->id }}" 
                                            ...
                                            {{ old('budget_line_id') == $line->id ? 'selected' : '' }}>
                ...
                <div class="card p-6">
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('projects.requisitions.index', $project) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="items[__INDEX__][specifications]" 
                              rows="2"
                              class="input-field"
                              placeholder="Technical specifications or requirements..."></textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-ink-700 mb-1">Type</label>
                    <select name="items[__INDEX__][item_type]" class="input-field">
